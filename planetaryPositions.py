...
def render(planets,date,frame):
    ...
    draw.text((0,0), "Date: "+date.strftime("%d/%m/%y"), fill="red", font=font) # Render info text
    ...
    image.save("simulationOutput/frame"+str(frame).zfill(4)+".png")
...
def simulate(startDate,cutoff=365): # Launch date is a datetime object and cutoff is the number of days to simulate
    ...
    for date in (startDate + datetime.timedelta(n) for n in range(cutoff)):
        ...
        currentFrame += 1
        render(planets,currentSimulatedDate,currentFrame)
        print("Rendered simulation for "+date.strftime("%d/%m/%y")+"...", end="\r")

    os.system('ffmpeg -i "simulationOutput/frame%04d.png" -r 30 -c:v libx264 -crf 22 simulation.mp4')
